...
if(isset($_POST) && !empty($_POST)){
	...
	if($category_id){
		redirect('../category_list.php','success','Category '.$act.'ed successfully.');
	} else {
		redirect('../category_list.php','error','Sorry! There was problem while '.$act.'ing category.');		
	}
	...
	if($id <= 0){
		redirect('../category_list.php', 'error','Invalid Category id provided.');
	}

	$category_detail = $category->getCategoryById($id);

	if(!$category_detail){
		redirect('../category_list.php', 'error','Category not found or has been already deleted.');
	}
	...
	if($del){
		...
		redirect('../category_list.php', 'success','Category deleted successfully.');
	} else {		
		redirect('../category_list.php', 'error','Sorry! There was problem while deleting category.');
	}
}
else {
	redirect('../category_list.php', 'error','Unauthorized access.');
}